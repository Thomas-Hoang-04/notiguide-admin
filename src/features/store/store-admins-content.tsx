...
interface StoreAdminsContentProps {
  active?: boolean;
  onAdminRemoved?: () => void;
  store: StoreDto | null;
  variant: "dialog" | "section";
}

export function StoreAdminsContent({
  active = true,
  onAdminRemoved,
  store,
  variant,
}: StoreAdminsContentProps) {
  ...
  const [loading, setLoading] = useState(active && !!store);
  ...
  async function handleUnassign() {
    ...
    try {
      ...
      setConfirmTarget(null);
      setAdmins((prev) =>
        prev.filter((admin) => admin.id !== confirmTarget.id),
      );
      onAdminRemoved?.();
    } catch (err) {
      if (err instanceof ApiError) {
  ...
  const listClassName =
    variant === "dialog"
      ? "mt-1 max-h-96 space-y-3 overflow-y-auto pr-1"
      : "max-h-72 space-y-2.5 overflow-y-auto pr-1";
  const itemClassName =
    variant === "dialog"
      ? "flex items-center justify-between gap-4 rounded-lg border border-border px-4 py-3.5"
  ...
  const unverifiedBadgeClassName =
    variant === "dialog"
      ? "mt-1 border-warning/40 bg-warning/15 text-warning text-xs dark:border-warning/50 dark:bg-warning/20"
      : "border-warning/40 bg-warning/15 text-warning text-xs dark:border-warning/50 dark:bg-warning/20";
  ...
  return (
    <>
      <div className={listClassName}>
        {loading &&
          ["a", "b", "c"].map((id) => (
            <div key={`skeleton-${id}`} className={itemClassName}>
        ...
        {!loading && admins.length === 0 && (
          <p className={emptyClassName}>{tStores("adminsDialogEmpty")}</p>
        )}

        {!loading &&
          admins.map((admin) => (
            <div key={admin.id} className={itemClassName}>
